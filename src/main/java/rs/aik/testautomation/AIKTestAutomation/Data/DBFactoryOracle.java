...
public class DBFactoryOracle  extends  DBFactory{
    ...
    public Connection createConnection() {
        ...
        String userName = DataManager.dbUser;
        String pwd = DataManager.dbPassword;
        ...
        try{
            if(conn.isValid(timeOut)) {
                ...
                for(int i=1; i<= parameters.length; i++)
                {
                    ...
                    else if (parameters[i-1] instanceof LocalDateTime) {
                        LocalDateTime ldt = (LocalDateTime)parameters[i-1];
    ...
    /**
     * Method for execution SQL with parameters
    ...
    public Boolean pingDB(Connection conn) {
        ...
        try{
            if(conn.isValid(timeOut)){
                ...
                ResultSet rs = statement.executeQuery("SELECT * FROM v$version");
                ping = true;
                rs.close();
                Log.info("Connection to [" + connectionString + "] is succesful!"); // log this
            }
        }
        catch(SQLException ex) {
            Log.error(ex.getMessage());
